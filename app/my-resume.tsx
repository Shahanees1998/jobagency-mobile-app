...
const BOTTOM_SHEET = {
  ...
  primaryButtonText: { fontSize: 16, fontWeight: '600' as const, color: APP_COLORS.white },
};
  ...
  return (
    <ScrollView style={styles.container} contentContainerStyle={styles.content} showsVerticalScrollIndicator={false}>
      ...
      {resume.languages.length === 0 ? (
        ...
        <View style={styles.sectionContent}>
          <Text style={styles.bodyText}>{resume.languages.join(', ')}</Text>
          <EditDeleteIcons
      ...
      {resume.certifications.length === 0 ? (
        <TouchableOpacity style={styles.placeholderBox} onPress={() => setCertsModal(true)} activeOpacity={0.7}>
          <Text style={styles.placeholderText}>Certifications & licenses will appear here</Text>
        </TouchableOpacity>
      ) : (
        <View style={styles.sectionContent}>
          <Text style={styles.bodyText}>{resume.certifications.join(', ')}</Text>
          <EditDeleteIcons
  ...
  return (
    <Modal visible={visible} transparent animationType="slide">
      <Pressable style={BOTTOM_SHEET.overlay} onPress={onClose}>
        <Pressable style={[BOTTOM_SHEET.sheet, { paddingBottom: insets.bottom + 24, maxHeight: '90%' }]} onPress={(e) => e.stopPropagation()}>
...
const styles = StyleSheet.create({
  ...
  placeholderText: { fontSize: 15, color: APP_COLORS.textMuted },
